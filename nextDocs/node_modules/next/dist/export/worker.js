...
async function exportPage({ parentSpanId , path , pathMap , distDir , outDir , appPaths , pagesDataDir , renderOpts , buildExport , serverRuntimeConfig , subFolders , serverless , optimizeFonts , optimizeCss , disableOptimizedLoading , httpAgentOptions , serverComponents  }) {
    ...
    return exportPageSpan.traceAsyncFn(async ()=>{
        ...
        try {
            ...
            const isAppPath = appPaths.some((appPath)=>appPath.startsWith(page + ".page"));
            if (serverless) {
                ...
                const { Component , ComponentMod , getServerSideProps , getStaticProps , pageConfig ,  } = await (0, _loadComponents).loadComponents(distDir, page, serverless, !!serverComponents, isAppPath);
                const ampState = {
                ...
                var ref1, ref2;
                const components = await (0, _loadComponents).loadComponents(distDir, page, serverless, !!serverComponents, isAppPath);
                const ampState = {
