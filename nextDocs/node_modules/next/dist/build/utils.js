...
async function isPageStatic({ page , distDir , serverless , configFileName , runtimeEnvConfig , httpAgentOptions , locales , defaultLocale , parentId , pageRuntime , edgeInfo  }) {
    ...
    return isPageStaticSpan.traceAsyncFn(async ()=>{
        ...
        if (pageRuntime === _constants.SERVER_RUNTIME.edge) {
            ...
            componentsResult = await (0, _loadComponents).loadComponents(distDir, page, serverless, false, false);
        }
        ...
        if (hasStaticProps && hasStaticPaths) {
            ({ paths: prerenderRoutes , fallback: prerenderFallback , encodedPaths: encodedPrerenderRoutes ,  } = await buildStaticPaths(page, componentsResult.getStaticPaths, configFileName, locales, defaultLocale));
        }
...
async function hasCustomGetInitialProps(page, distDir, isLikeServerless, runtimeEnvConfig, checkingApp) {
    require("../shared/lib/runtime-config").setConfig(runtimeEnvConfig);
    const components = await (0, _loadComponents).loadComponents(distDir, page, isLikeServerless, false, false);
