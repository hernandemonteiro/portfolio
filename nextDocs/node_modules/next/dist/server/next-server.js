...
class NextNodeServer extends _baseServer.default {
     ...
     */ if (this.renderOpts.optimizeFonts) {
        ...
        if (!options.dev) {
            ...
            (0, _loadComponents).loadComponents(this.distDir, "/_document", this._isLikeServerless, false, false).catch(()=>{});
            (0, _loadComponents).loadComponents(this.distDir, "/_app", this._isLikeServerless, false, false).catch(()=>{});
        }
    }
    ...
    async findPageComponents({ pathname , query , params , isAppPath  }) {
        let paths = [
            ...
            query.amp ? (isAppPath ? (0, _appPaths).normalizeAppPath(pathname) : (0, _normalizePagePath).normalizePagePath(pathname)) + ".amp" : null,
            pathname, 
        ].filter(Boolean);
        ...
        for (const pagePath of paths){
            try {
                const components = await (0, _loadComponents).loadComponents(this.distDir, pagePath, !this.renderOpts.dev && this._isLikeServerless, !!this.renderOpts.serverComponents, isAppPath);
   ...
   */ async runMiddleware(params) {
    ...
    async runEdgeFunction(params) {
        ...
        const query = (0, _querystring).urlQueryToSearchParams(params.query).toString();
        ...
        if ((0, _utils1).isDynamicRoute(normalizedPathname)) {
            const routeRegex = (0, _routeRegex).getNamedRouteRegex(params.page);
            normalizedPathname = (0, _utils4).interpolateDynamicPath(params.page, Object.assign({}, params.params, params.query), routeRegex);
        }
        const url = `${(0, _requestMeta).getRequestMeta(params.req, "_protocol")}://${this.hostname}:${this.port}${locale ? `/${locale}` : ""}${normalizedPathname}${query ? `?${query}` : ""}`;
        if (!url.startsWith("http")) {
            throw new Error("To use middleware you must provide a `hostname` and `port` to the Next.js Server");
        }
        const result = await (0, _sandbox).run({
