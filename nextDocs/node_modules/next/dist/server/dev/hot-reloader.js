    ...
    async getWebpackConfig(span) {
        ...
        return webpackConfigSpan.traceAsyncFn(async ()=>{
            const pagePaths = !this.pagesDir ? [] : await webpackConfigSpan.traceChild("get-page-paths").traceAsyncFn(()=>Promise.all([
                    (0, _findPageFile).findPageFile(this.pagesDir, "/_app", rawPageExtensions, false),
                    (0, _findPageFile).findPageFile(this.pagesDir, "/_document", rawPageExtensions, false), 
                ]));
        ...
        for (const config1 of this.activeConfigs){
            ...
            config1.entry = async (...args)=>{
                ...
                await Promise.all(Object.keys(_onDemandEntryHandler.entries).map(async (entryKey)=>{
                    ...
                    const isServerComponent = isEntry ? _utils2.serverComponentRegex.test(entryData.absolutePagePath) : false;
                    ...
                    await (0, _entries).runDependingOnPageType({
                        ...
                        onEdgeServer: ()=>{
                            ...
                            const appDirLoader = isApp && this.appDir ? (0, _entries).getAppEntry({
                                name: bundlePath,
